	...
	function showHandler(node) {
		...
		if (def.color) {
			...
			var setVal = function() {
				...
				var a = inp.spectrum('get').alpha;
		    	inp.val( 'rgba('+c.r+','+c.g+','+c.b+','+a+')' );
		    	inp.trigger('change');
			};
			inp.spectrum({
			    color: vInit,
...
qgStyleEditor.gradientHandler = function(colorPalette) {
	...
	var setVal = function() {
		...
		var startA = startInp.spectrum('get').alpha;
		var startColor = 'rgba('+startC.r+','+startC.g+','+startC.b+','+startA+')';

		var endC = endInp.spectrum('get').toRgb();
		var endA = endInp.spectrum('get').alpha;
		var endColor = 'rgba('+endC.r+','+endC.g+','+endC.b+','+endA+')';

		hidden[0].value = '-webkit-linear-gradient(top,'+startColor+','+endColor+')';
...
qgStyleEditor.shadowHandler = function(colorPalette, hasSpread) {
	...
	var setVal = function() {
		...
		var alpha = colorInp.spectrum('get').alpha;
