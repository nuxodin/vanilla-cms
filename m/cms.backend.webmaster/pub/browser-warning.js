...
//    window.error_report_count = 1000; // dont send errors
    var iv = setInterval(function(){
        ...
        var browsers = [
            { name: 'firefox', link: 'https://getfirefox.com' },
            { name: 'chrome',  link: 'https://www.google.com/chrome/browser/desktop/index.html' },
            { name: 'opera',   link: 'https://www.opera.com/' }
        ];
        var OS = '';
        ...
        if (OS === 'Windows') browsers.push( { name: 'edge',   link: 'https://www.microsoft.com/en-us/windows/microsoft-edge/microsoft-edge' } );
        if (OS === 'Mac')     browsers.push( { name: 'safari', link: 'https://support.apple.com/de-ch/HT204416' } );
        var html =
        '<div style="position:fixed; z-index:9999; top:0; left:0; right:0; padding:12px; background:#ff8; border-bottom:1px solid #777; font-size:14px; color:#000">'+
            '<table style="vertical-align:middle; width:100%; margin-bottom:.6em">'+
                '<tr>'+
                    ...
                    '<td style="text-align:right">'+
                        '<button id=browserWarningIgnoreBtn>ignorieren</button>'+
            '</table>';
        ...
        document.getElementById('browserWarningIgnoreBtn').addEventListener('click',function(){
