...
if ($Cont->SET['Einblenden']['Haftungsausschluss']->v) {
	$Text = $Cont->Text('Haftungsausschluss');
	if (!trim($Text)) {
		$Text->set('<h2>Haftungsausschluss</h2>
		Der Autor übernimmt keinerlei Gewähr hinsichtlich der inhaltlichen Richtigkeit, Genauigkeit, Aktualität, Zuverlässigkeit und Vollständigkeit der Informationen.
		Haftungsansprüche gegen den Autor wegen Schäden materieller oder immaterieller Art, welche aus dem Zugriff oder der Nutzung bzw. Nichtnutzung der veröffentlichten Informationen, durch Missbrauch der Verbindung oder durch technische Störungen entstanden sind, werden ausgeschlossen.
		Alle Angebote sind unverbindlich. Der Autor behält es sich ausdrücklich vor, Teile der Seiten oder das gesamte Angebot ohne gesonderte Ankündigung zu verändern, zu ergänzen, zu löschen oder die Veröffentlichung zeitweise oder endgültig einzustellen.<br><br>');
	}
	echo '<div '.($Cont->edit? 'contenteditable cmstxt='.$Text->id : '').'>'.$Text.'</div>';
}

if ($Cont->SET['Einblenden']['Haftungsausschluss Inhalte, Links']->v) {
	$Text = $Cont->Text('Haftungsausschluss Links');
	if (!trim($Text)) {
		$Text->set('<h2>Haftung für Links</h2>
		Verweise und Links auf Webseiten Dritter liegen ausserhalb unseres Verantwortungsbereichs Es wird jegliche Verantwortung für solche Webseiten abgelehnt. Der Zugriff und die Nutzung solcher Webseiten erfolgen auf eigene Gefahr des Nutzers oder der Nutzerin.<br><br>');
	}
	echo '<div '.($Cont->edit? 'contenteditable cmstxt='.$Text->id : '').'>'.$Text.'</div>';
}

if ($Cont->SET['Einblenden']['Urheberrecht']->v) {
	$Text = $Cont->Text('Urheberrecht');
	if (!trim($Text)) {
		$Text->set('<h2>Urheberrechte</h2>
		Die Urheber- und alle anderen Rechte an Inhalten, Bildern, Fotos oder anderen Dateien auf der Website gehören ausschliesslich dem Betreiber der Website oder den speziell genannten  Rechtsinhabern. Für die Reproduktion jeglicher Elemente ist die schriftliche Zustimmung der Urheberrechtsträger im Voraus einzuholen.<br><br>');
	}
	echo '<div '.($Cont->edit? 'contenteditable cmstxt='.$Text->id : '').'>'.$Text.'</div>';
}

if ($Cont->SET['Einblenden']['Datenschutzerklärung']->v) {
	$Text = $Cont->Text('Datenschutzerklärung');
	if (!trim($Text)) {
		$Text->set('<h2>Datenschutz</h2>
...
if ($Cont->SET['Einblenden']['Cookies']->v) {
	$Text = $Cont->Text('Cookies');
	if (!trim($Text)) {
		$Text->set('<h2>Cookies</h2>
...
if ($Cont->SET['Einblenden']['Datenschutzerklärung Facebook']->v) {
	$Text = $Cont->Text('Datenschutzerklärung Facebook');
	if (!trim($Text)) {
		$Text->set('<h2>Datenschutz: Facebook</h2>
...
if ($Cont->SET['Einblenden']['Datenschutzerklärung Twitter']->v) {
	$Text = $Cont->Text('Datenschutzerklärung Twitter');
	if (!trim($Text)) {
		$Text->set('<h2>Datenschutz: Twitter</h2>
		...
		Ihre Datenschutzeinstellungen bei Twitter können Sie in den Konto-Einstellungen unter <a href="http://twitter.com/account/settings" target="_blank">http://twitter.com/account/settings</a> ändern. Bei Fragen wenden Sie sich an <a href="mailto:privacy@twitter.com">privacy@twitter.com</a>.<br><br>');
	}
	echo '<div '.($Cont->edit? ' contenteditable cmstxt='.$Text->id : '').'>'.$Text.'</div>';
}

if ($Cont->SET['Einblenden']['Datenschutzerklärung Google Adsense']->v) {
	$Text = $Cont->Text('Datenschutzerklärung Google Adsense');
	if (!trim($Text)) {
		$Text->set('<h2>Datenschutz: Google Adsense</h2>
...
if ($Cont->SET['Einblenden']['Datenschutzerklärung Google Analytics']->v) {
	$Text = $Cont->Text('Datenschutzerklärung Google Analytics');
	if (!trim($Text)) {
		$Text->set('<h2>Datenschutz: Google Analytics</h2>
...
if ($Cont->SET['Einblenden']['Datenschutzerklärung Google +1 (Google Plus)']->v) {
	$Text = $Cont->Text('Datenschutz: Google Plus');
	if (!trim($Text)) {
		$Text->set('<h2>Datenschutz: Google Plus</h2>
...
if ($Cont->SET['Einblenden']['Datenschutzerklärung Google Remarketing']->v) {
	$Text = $Cont->Text('Datenschutz: Google Remarketing');
	if (!trim($Text)) {
		$Text->set('<h2>Datenschutz: Google Remarketing</h2>
		Diese Website verwendet die Remarketing- bzw. "Ähnliche Zielgruppen"-Funktion der Google Inc. ("Google"). Sie können so zielgerichtet mit Werbung angesprochen werden, indem personalisierte und interessenbezogene Anzeigen geschaltet werden, wenn Sie andere Webseiten im sog. "Google Display-Netzwerk" besuchen. "Google Remarketing" bzw. die Funktion "Ähnliche Zielgruppen" verwendet dafür sog. "Cookies", Textdateien, die auf Ihrem Computer gespeichert werden und die eine Analyse der Benutzung der Website durch Sie ermöglichen. Über diese Textdateien werden Ihre Besuche sowie anonymisierte Daten über die Nutzung der Website erfasst. Personenbezogene Daten werden dabei nicht gespeichert. Besuchen Sie eine andere Webseite im sog. "Google Display-Netzwerk" werden Ihnen ggf. Werbeeinblendungen angezeigt, die mit hoher Wahrscheinlichkeit zuvor auf unserer Website aufgerufene Produkt- und Informationsbereiche berücksichtigen. Sie können die Installation der Cookies durch eine entsprechende Einstellung Ihrer Browser Software verhindern; wir weisen Sie jedoch darauf hin, dass Sie in diesem Fall gegebenenfalls nicht sämtliche Funktionen dieser Website voll umfänglich nutzen können. Durch die Nutzung dieser Website erklären Sie sich mit der Bearbeitung der über Sie erhobenen Daten durch Google in der zuvor beschriebenen Art und Weise und zu dem zuvor benannten Zweck einverstanden.<br><br>');
	}
	echo '<div '.($Cont->edit? 'contenteditable cmstxt='.$Text->id : '').'>'.$Text.'</div>';
}
