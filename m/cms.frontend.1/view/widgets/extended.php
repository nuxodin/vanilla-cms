...
<div class=qgCmsFront1DiversManager pid=<?=$Cont?>>
    ...
    <table>
        ...
        <tr>
            <td> <?=L('Bezeichner')?> (<?=L('Layout-Position')?>):
            <td> <input class=-name value="<?=hee($Cont->vs['name'])?>" style="width:250px">
        <tr>
            <td> <?=L('Basis')?>:
            <td> <input class=-basis type=qgcms-page value="<?=hee($Cont->vs['basis'])?>" style="width:250px">
    </table>
	<br>
